...
- [x] **4.1** Tag architecture: `pilot-member`, `founding-3mo`, `activated`, `at-risk`, `glp1-risk-high/elevated/low`, `glp1-quiz-complete`, `glp1-prestart`
      ...
      → Full architecture documented 2026-08-01 in `glp1-tags.md` — now fourteen tags in four groups (adds `founding-monthly`, `src-list`, `kit-viewed`, `sms-ok`, `onboarded`, `glp1-prestart-consult`, and `glp1-quiz-started` to the list above), with application timing, Sunday cadence, and the at-risk add/remove rule. Doc-vs-GHL reconciliation before launch is item 5.5.
...
- [ ] **5.2** DRESS REHEARSAL: real live-mode purchase (own card) → tags → provisioning → welcome email → first login → wizard → baseline → completion screen ("Your baseline is set!") → dashboard → Day-0 messages arrive
      ...
      → **2026-08-08 — rehearsal found and closed a display bug on the Today screen (app fix allowed under Standing Rule 1 — baseline → dashboard is the paying member's core journey, same class as the July 28 Fix A precedent).** Overview Statistics showed the baseline weight/waist misconverted (215 lbs → "474 lbs", 36 in → "14 in"): entries written without `normalizedValue` (the row shape the onboarding baseline step produces) were run through the kg/cm→display conversion despite carrying values already in the member's units. Every US-units member would have hit this on day 0. Fixed in `0e26af0` (read-side only — OverviewStatistics now matches the fallback contract of every other reader; server-side normalization deliberately deferred to the pilot weeks as BACKLOG.md item 21). **Verified on the rehearsal account in production:** Weight latest 214 / Avg 215 / Range 214-215 across 2 entries — including the mixed case of a raw baseline row (215) aggregating with a normalized modal entry (214), which no other prod account can currently produce — Waist 36 / Avg 36 / Range 36-36, Health Metrics 214 lbs and 36.0 in, Trends unchanged. Likely also explains BACKLOG.md item 11's July 21 "463 lbs / 14-in waist" sighting (exactly this bug's output for an entered 210/36).
      → **Item still open 2026-08-08:** remaining checks are Sequence 1's day-1 email and the `kit-viewed` tag (due 2026-08-09), then account cleanup. The rehearsal account is deliberately retained until then.
...
- [x] **5.4** Pre-starter results path: quiz completions choosing Q1 "Not currently, but I'm considering it" (tagged `glp1-prestart`) currently answer all eight questions — several of which assume they're already losing weight on the medication — then land on a normal results page with a score and risk band that aren't true of them, and a CTA for a $49/mo program built around a drug they aren't taking. Fix: pre-starters get their own results content with a consult-funnel CTA instead of the founding-member sales page — someone considering a GLP-1 is a good lead for the practice, and a consult is a better offer than "come back later."
      ...
      → **Done 2026-08-05, all in ScoreApp + GHL.** (1) Fourth result page, "Pre-starter result" at /considering-a-glp-1, cloned from Low risk result with the score line removed entirely — explains why a risk score isn't meaningful for someone not yet taking a GLP-1 and offers a screening call instead of the founding-member program. (2) Result-page routing switched from Overall score tier to Audience: rows evaluate top-down on first match, Pre-starter sits in position one so it wins regardless of score, and the three risk audiences each gained a condition requiring question 1 "Is not" the pre-starter answer — all four audiences mutually exclusive. Q1 is required, so nobody reaches the end unmatched; Low risk result remains the default catch-all. (3) New GHL calendar "Thinking about a GLP-1? Talk to Dr. Larson" — 20 minutes, Wednesdays 11am–2pm Pacific, $29 in Stripe live mode, credited against the first month if they enroll; deliberately a different day from the existing Metabolic OS Screening calendar so the two don't compete for the same slots. (4) Redirect join.theadaptlab.com/consult → the calendar's permanent booking URL, so the page carries a clean link and the target can change without editing the page. (5) Workflow "Consult booked — tag glp1-prestart-consult" applies the new tag on any booking from that calendar, so pre-starter → consult conversion is measurable (see `glp1-tags.md`).
      → **Verified with two live completions:** a pre-starter landed on the new page with no score line and the button reached a working booking widget; a regular respondent still landed on their risk page with the score line intact and the CTA reaching /founding.
      → **Side effect worth recording:** the audience tag rules run off the same audiences, so a pre-starter no longer receives a risk band tag at all (previously `glp1-prestart` landed on top of whatever band their score earned). Nobody in GHL now carries a risk label that isn't true of them.
- [x] **5.5** Reconcile `glp1-tags.md` against the live tag list in GoHighLevel
