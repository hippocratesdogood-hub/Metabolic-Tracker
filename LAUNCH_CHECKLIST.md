...
- [x] **4.1** Tag architecture: `pilot-member`, `founding-3mo`, `activated`, `at-risk`, `glp1-risk-high/elevated/low`, `glp1-quiz-complete`, `glp1-prestart`
      ...
      → Full architecture documented 2026-08-01 in `glp1-tags.md` — now thirteen tags in four groups (adds `founding-monthly`, `src-list`, `kit-viewed`, `sms-ok`, `onboarded`, and `glp1-prestart-consult` to the list above), with application timing, Sunday cadence, and the at-risk add/remove rule
