...
**Near-certain relation to item 21 / the 2026-08-08 Overview Statistics bug (assessment upgraded 2026-08-08):** the "463 lbs / 14-inch waist" observed in the July 21 T5 journey are exactly the values the Overview Statistics display bug produces for an entered 210 lbs / 36 in (210 × 2.2046 = 463; 36 ÷ 2.54 = 14.2). Initially flagged as merely possible; upgraded to near-certain after the same misconversion was reproduced and fixed (`0e26af0`) on a live production account during the 5.2 dress rehearsal — an entered 215/36 rendered as 474/14 by the identical mechanism. The likely reading of the July sighting is that the tester entered plausible values and *read* the misconverted output off the Today screen, so this item's premise ("wizard accepted impossible values") largely described the display bug, not missing validation. Input validation is still worth adding on its own merits, but re-verify what the wizard actually accepts before sizing this work — the observed evidence for the original claim is now mostly accounted for.
